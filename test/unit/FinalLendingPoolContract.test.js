...
describe("FinalLendingPoolContract", function () {
    ...
    beforeEach(async () => {
        ...
        // Deploy FinalLendingPoolContract
        ...
        lendingPoolContract = await FinalLendingPoolContractFactory.deploy(
            ...
            lpToken.address,
            {
                gasLimit: 5000000, // Adjust based on contract complexity
                gasPrice: ethers.utils.parseUnits("10", "gwei"),
            }
        );
        await lendingPoolContract.deployed();

        // Transfer USDT tokens to the lending pool contract
        await usdtToken.transfer(lendingPoolContract.address, initialUSDTBalance);

        // Set LendingPool in LPToken
    ...
    it("should revert when the LP token burn amount exceeds the user's LP token balance", async () => {
        ...
        const ethWithdrawAmount = ethers.utils.parseEther("2"); // Within deposited Ether
        const lpBurnAmount = 10; // Exceeds LP token balance
    ...
    it("should allow borrowing funds based on collateral and price feed", async () => {
        const depositCollateralAmount = ethers.utils.parseEther("10"); // 10 ETH
        ...
        const newPrice = ethers.utils.parseUnits("2500", 8); // ETH/USDT = 2500
        await mockV3Aggregator.updateAnswer(newPrice);

        const borrowAmount = ethers.utils.parseUnits("10000", 6); // 10,000 USDT

        // Approve lending pool to transfer USDT on behalf of user1
